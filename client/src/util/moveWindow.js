...
Vue.directive('dialogDrag', {
  bind(el, binding, vnode, oldVnode) {
    ...
    let nowMarginTop = 0;
    // 获取弹框头部（这部分可双击全屏）
    const dialogHeaderEl = el.querySelector('.move-window-header');
    // 弹窗
    const dragDom = el;
    ...
    dragDom.onmousemove = function(e) {
      ...
      if (
        ...
        el.scrollTop + e.clientY > dragDom.offsetTop + dragDom.clientHeight - 10
      ) {
        dragDom.style.cursor = 'se-resize';
      } else if (
        e.clientX >
        dragDom.offsetLeft + dragDom.clientWidth - 10
